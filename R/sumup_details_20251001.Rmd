...
.comment-tooltip {
  ...
  border-radius: 4px;
  min-width: 240px;
  max-width: 360px; /* wider for readability */
...
$(function() {
  ...
  domainTopics.forEach(topic => {
    (topic.feedback || []).forEach(f => {
      ...
      const coschap = raw === "" ? "(Onbekend)" : raw;   // canonical label
  ...
  const coschapRank = {};
  coschaps.forEach((c, i) => coschapRank[c] = i + 1);
...
domainTopics.forEach(topic => {
  (topic.feedback || []).forEach(f => {
    const raw = (f.coschap ?? "").toString().trim();
    const coschap = raw === "" ? "(Onbekend)" : raw;
...
coschapsSorted.forEach(c => {
  ...
  filterHtml += `
    <label style="margin-right:10px; display:inline-block; margin-top:5px;">
      <input type="checkbox" class="coschap-filter" value="${c}" checked> ${c}${rangeText}
    </label>`;
});
$("#filter-container").html(filterHtml);
  ...
  function renderCommentIcon(comment){
    if (!comment) return "";
  ...
  function renderFeedbackTable(feedback, tableIdx){
    ...
    rows.forEach(r => {
      html += `
        <tr>
          <td style="padding:6px;">${renderPolarityBar(r.polarity)}</td>
          <td style="padding:6px;">${escapeHtml(r.sentence)} ${renderCommentIcon(r.comment)}</td>
    ...
    setTimeout(() => {
      $(`#${tableId}`).DataTable({
        ...
        columnDefs: [
          ...
          { orderable: false, targets: [0, 5] },   // first (0) and last (5) columns not sortable
          { orderable: true,  targets: [1, 2, 3, 4] } // the rest remain sortable
        ]
      });
    }, 0);
